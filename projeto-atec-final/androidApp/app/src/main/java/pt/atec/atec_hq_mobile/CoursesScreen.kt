        ...
        Row(
            ...
            Surface(
                ...
                Box(contentAlignment = Alignment.Center) {
                    Icon(imageVector = Icons.Default.List, contentDescription = null, tint = AtecBlue, modifier = Modifier.size(24.dp))
                }
            }
            Spacer(modifier = Modifier.width(16.dp))
            Column {
                Text(text = course.nome, style = MaterialTheme.typography.titleMedium, color = AtecText, fontWeight = FontWeight.Bold)
                Text(text = "${course.area} • ${course.duracao_horas}h", style = MaterialTheme.typography.bodyMedium, color = Color.Gray)
            }
        }
    }
}
